...
class _InboundScanScreenState extends State<InboundScanScreen> {
  ...
  // ✅ autoStart를 false로 두고 수동으로 카메라 제어
  ...
  void initState() {
    ...
    // ✅ 이전 탭에서 카메라를 끌 시간을 0.3초 확보한 뒤 안전하게 켭니다.
  ...
  @override
  void dispose() {
    // ✅ 탭을 나갈 때 멱살 잡고 있던 카메라를 무조건 놔줍니다.
    controller.stop();
    controller.dispose();
    super.dispose();
  }
  ...
  void reset({bool keepLastResult = false}) {
    setState(() {
      product = null; location = null; scanningProduct = true; isProcessingScan = false; isSubmitting = false; lastScannedRaw = null;
  ...
  Future<void> onDetect(BarcodeCapture capture) async {
    ...
    try {
      // ✅ 여기서부터: 스마트 QR(홈페이지 주소)인지, 기존 구형 QR인지 구분해서 읽는 로직
    ...
    // ✅ 여기까지 수정됨. 아래 로직은 대표님 코드와 100% 동일하게 유지!
  ...
  Widget _buildInfoPanel() {
    final p = product; final l = location; final canConfirm = (p != null && l != null && !isSubmitting);
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [const Text('스캔 결과', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)), const Divider(), const SizedBox(height: 8), Text('📦 제품: ${p != null ? '${p['name']} / ${p['qty']}개' : '대기중...'}', style: TextStyle(color: p != null ? Colors.black : Colors.grey)), const SizedBox(height: 8), Text('📍 위치: ${l != null ? l['locationCode'] : '대기중...'}', style: TextStyle(color: l != null ? Colors.black : Colors.grey)), if (lastBatchId != null) ...[const SizedBox(height: 16), Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: lastDuplicate == true ? Colors.red.shade50 : Colors.green.shade50, borderRadius: BorderRadius.circular(8)), child: Text(lastDuplicate == true ? '⚠️ 중복 입고 (기존 Batch: $lastBatchId)' : '✅ 입고 완료 (Batch: $lastBatchId)', style: TextStyle(fontWeight: FontWeight.bold, color: lastDuplicate == true ? Colors.red.shade700 : Colors.green.shade700)))], const SizedBox(height: 20), FilledButton.icon(onPressed: canConfirm ? _confirmInbound : null, style: FilledButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16)), icon: isSubmitting ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.check_circle), label: Text(isSubmitting ? '업데이트 중...' : '입고 확정', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))), const SizedBox(height: 12), OutlinedButton.icon(onPressed: (scanningProduct || isSubmitting) ? null : () { setState(() { location = null; lastBatchId = null; lastDuplicate = null; lastScannedRaw = null; }); _setStatus('위치 QR을 다시 스캔하세요', Colors.orange); controller.start(); }, icon: const Icon(Icons.qr_code_scanner), label: const Text('위치만 다시 스캔'))]))),
        ],
      ),
    );
  }
